...
from .views import (
    CustomLoginView, RegisterView, MeView,
    RoomListCreateView, RoomDetailView,
    AvailableRoomsView, CancelReservationView, UserReservationsView,
    ReservationListCreateView, ReservationDetailView, RoomAvailabilityView, AdminStatsView
)

urlpatterns = [
    # Room Availability (keep this first)
    path('rooms/<int:room_id>/availability/', RoomAvailabilityView.as_view(), name='room-availability'),
    
    # Authentication - Use only CustomLoginView (remove TokenObtainPairView)
    path('register/', RegisterView.as_view(), name='register'),
    path('login/', CustomLoginView.as_view(), name='login'),  # ← Only one login path
