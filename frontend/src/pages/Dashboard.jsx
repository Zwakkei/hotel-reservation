  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-indigo-900 to-blue-900 py-8">
      <div className="container mx-auto px-4">
        <div className="bg-gradient-to-r from-amber-900 to-blue-900 rounded-2xl p-8 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Link to="/rooms" className="bg-blue-900/40 rounded-2xl p-6 text-center border border-amber-500/20 hover:shadow-lg hover:shadow-amber-500/10 transition">
            <div className="text-5xl mb-4">🏨</div>
            <h3 className="text-xl font-bold text-white">Browse Rooms</h3>
            <p className="text-gray-300 mt-2">Find your perfect room</p>
          </Link>
          <Link to="/reservations" className="bg-blue-900/40 rounded-2xl p-6 text-center border border-amber-500/20 hover:shadow-lg hover:shadow-amber-500/10 transition">
            <div className="text-5xl mb-4">📅</div>
            <h3 className="text-xl font-bold text-white">My Bookings</h3>
            <p className="text-gray-300 mt-2">View your reservations</p>
          </Link>
          <div className="bg-blue-900/40 rounded-2xl p-6 text-center border border-amber-500/20">
            <div className="text-5xl mb-4">👤</div>
            <h3 className="text-xl font-bold text-white">Account</h3>
            <p className="text-gray-300 mt-2">{user?.email}</p>
          </div>
        </div>
        <div className="mt-8 bg-blue-900/40 rounded-2xl p-6 border border-amber-500/20">
          <h2 className="text-xl font-bold text-white mb-4">Recent Bookings</h2>
          {reservations.length === 0 ? (
            <p className="text-gray-300">No bookings yet. <Link to="/rooms" className="text-amber-400">Book a room</Link></p>
          ) : (
            reservations.slice(0, 3).map(res => (
              <div key={res.id} className="border-b border-cyan-800 py-3 flex justify-between">
                <span className="text-white">{res.room?.name}</span>
                <span className="text-gray-300">{res.check_in} to {res.check_out}</span>
